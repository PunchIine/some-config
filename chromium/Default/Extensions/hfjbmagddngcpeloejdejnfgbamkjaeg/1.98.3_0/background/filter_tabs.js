...
}),u.hu=u.mu=u.bu=u.mayRequireActiveTab=u.getNecessaryCurTabInfo=u.ku=u.getNearArrIndex=u.gu=u.wu=u.xu=u.getTabRange=void 0,
u.getTabRange=function(n,u,e,t){return c(n,u,e,r.k,t,r.Ie.limited)},c=function(n,u,r,e,t,l){var i,a=e>0
;return t&&(e+=a?t:-t),
...
a.overrideOption("$limit",r.k),r.k=r.k>0?9999:-9999),l(i,n?[s,o,d]:[o+1===d||r.k>0&&s!==o?s:d-1,o,d],c)
},d=r.Ie.filter,h=d&&/(^|[&+])limit(ed)?=count\b/.test(d+"")
;i?0===i.length||Math.abs(r.k)>1||h?0===i.length||h||false?((f=i[0]?i[0].windowId:r.J)>=0?t.Wn(t.An.get,f,{populate:true
}):t.Wn(t.getCurWnd,true)).then(function(n){s(n?n.tabs:[])}):s(i):e?i[0].index+r.k<0?t.Mn(s):t.Bn.query({
windowId:i[0].windowId,index:i[0].index+r.k},function(e){
return e&&e.length&&(true===o||t.Fn(e[0])&&(!o||o(e[0])))&&(!d||u.mu(i[0],e,d).length>0)?r.k<0?l([e[0],i[0]],[0,1,n?2:1],c):l([i[0],e[0]],[n?0:1,0,2],c):t.Mn(s),
...
return e&&e.length?e[u.getNearArrIndex(e,n.index+(r?1:-1),r)]:null}):null}):Promise.resolve(null)},u.gu=f,
...
u.ku=function(n,u){1===Math.abs(n)?t.getCurTab(function(r){var e=r[0].index+n;e>=0?t.Bn.query({windowId:r[0].windowId,
...
return n&&n[0]||null}):r?Promise.resolve(l.Hn(null,r>1)).then(function(n){return n?{url:n}:null}):null},
...
var l,a,c,o,s,h,v,m,b,k,g,p,w,P,j,y,q,A=u.split("=",1)[0],N=A.includes("."),O=!N&&A.endsWith("!"),$=N?"":(O?A.slice(0,-1):A)||u,z=u.slice(N?0:A.length+("="===u.charAt(A.length+1)?2:1)),B=z&&e.in(z),C="same"===B||"cur"===B||"current"===B,D=null
;switch($){case"title":case"title*":l=B||n&&n.title,a=B&&"/"===B[0]?B.lastIndexOf("/"):0,
D=(c=a>1?e.nn(B.slice(1,a),B.slice(a+1).replace(/g/g,""),0):null)?function(n){return c.test(n.title||"")}:l?function(n){
return(n.title||"").includes(l)}:D;break;case"url":case"urlhash":case"url+hash":case"url-hash":case"hash":o=null,
"url"===$&&B?o=i.rt(B):(s=n?t.getTabUrl(n):null,h=$.includes("hash"),o=s?i.rt(":"+(h?s:s.split("#",1)[0])):null),
D=o?function(n){return i.ot(o,t.getTabUrl(n))}:D;break;case"host":case"":
D=(v=B||($&&n?null===(f=e.sn(t.getTabUrl(n)))||void 0===f?void 0:f.host:""))?function(n){var u
;return v===(null===(u=e.sn(t.getTabUrl(n)))||void 0===u?void 0:u.host)}:D;break;case"active":
D=null!=(m=d(B,1))?function(n){return n.active===m}:D;break;case"new":case"old":case"visited":b=d(B)===("new"!==$),
D=function(n){return r.T.has(n.id)===b};case"discarded":case"discard":D=null!=(k=!C&&d(B,1))?function(n){
...
case"limited":q="count"===B?r.Ie.$limit||r.k:parseInt(B)||0,"min"===$?M=q:"max"===$?_=q:x=q||1,D=function(){return true}
...
!(m=v.length)||M>0&&m<M||_>0&&m>_?(r.Ie&&r.Ie.$else||l.showHUD(m?"".concat(m," tabs found but expect ").concat(m<M?M:_):"No tabs matched the filter parameter"),
[]):(x&&((b=n?h.indexOf(n):-1)<0&&(k=n?n.id:r.L,b=h.findIndex(function(n){return n.id===k})),
b>=0?((p=(g=v.findIndex(function(n){return h.indexOf(n)>=b}))>=0&&h.indexOf(v[g])>b)&&v.splice(g,0,null),
w=c(g>=0?g:m-1,m,0,r.k>0?x:-x,p?1:0,false),v=v.slice(w[0],w[1]),p&&(v=v.filter(function(n){return!!n
